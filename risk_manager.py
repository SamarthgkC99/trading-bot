# risk_manager.py - Hybrid TP/SL Risk Management System + GitHub Storage
...
def load_risk_config():
    """Load risk configuration from GitHub, with local file fallback."""
    print("--- Attempting to load risk_config.json from GitHub ---")
    try:
        storage = GitHubStorage()
        config = storage.read_file("risk_config.json")
        if config:
            print("--- Risk config loaded from GitHub successfully. ---")
            return config
    except Exception as e:
        print(f"Could not load risk_config from GitHub: {e}. Using fallback.")
        
    # Fallback to local file or default
    if not os.path.exists(RISK_CONFIG_FILE):
        print("--- Local risk config not found. Creating default. ---")
        save_risk_config(DEFAULT_RISK_CONFIG) # This will save locally
...
def save_risk_config(config):
    """Save risk configuration to GitHub and local file."""
    print("--- Saving risk configuration ---")
    try:
        storage = GitHubStorage()
        if storage.write_file("risk_config.json", config):
            print("--- Risk config saved to GitHub successfully. ---")
    except Exception as e:
        print(f"Error saving risk_config to GitHub: {e}")
...
# ====== MODIFIED FUNCTION ======
def load_risk_state():
    """Load daily risk tracking state from GitHub, with local file fallback."""
    print("--- Attempting to load risk_state.json from GitHub ---")
    try:
        storage = GitHubStorage()
        state = storage.read_file("risk_state.json")
        if state:
            # Check if we need to reset (new day)
            last_reset = datetime.fromisoformat(state["last_reset"])
            config = load_risk_config() # Use the freshly loaded config
            ...
            if today > last_reset_date or (today == last_reset_date and now.hour >= reset_hour and last_reset.hour < reset_hour):
                print("--- Daily reset condition met. Resetting state. ---")
                state = reset_daily_state()
            return state
    except Exception as e:
        print(f"Could not load risk_state from GitHub: {e}. Using fallback.")

    # Fallback to local file or reset
    if not os.path.exists(RISK_STATE_FILE):
        print("--- Local risk state not found. Creating default. ---")
...
def save_risk_state(state):
    """Save risk state to GitHub and local file."""
    print("--- Saving risk state ---")
    try:
        storage = GitHubStorage()
        if storage.write_file("risk_state.json", state):
            print("--- Risk state saved to GitHub successfully. ---")
    except Exception as e:
        print(f"Error saving risk_state to GitHub: {e}")
